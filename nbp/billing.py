...
def subscribe_success():
    ...
    meta = (_.get("metadata") or {}) if isinstance(_, dict) else {}
    ...
    def compute_redirect(plan: str, counties: list[str]) -> str:
        ...
        # Example combined route via querystring:
...
def billing_portal():
    """
    Optional: customer self-serve portal.
    Requires session['stripe_customer_id'] to be set in subscribe_success().
    """
    try:
        s = use_stripe()
        ...
        portal = s.billing_portal.Session.create(customer=cust_id, return_url=base)
