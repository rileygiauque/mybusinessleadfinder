...
def billing_portal():
    ...
    try:
        ...
        if not cust_id and session.get("user_email"):
            ...
            if subscriber and subscriber.stripe_customer_id:
                cust_id = subscriber.stripe_customer_id
                session["stripe_customer_id"] = cust_id  # Cache it
        
        if not cust_id:
            # Last resort: create a portal session link via email lookup
        ...
        portal = s.billing_portal.Session.create(
            customer=cust_id,
            return_url=base + "/new-business/florida/"  # Where they return after managing
        )
